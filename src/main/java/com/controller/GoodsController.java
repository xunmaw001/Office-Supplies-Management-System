...
public class GoodsController {
    ...
    */
    ...
    public R renting(Integer num,Integer ids, HttpServletRequest request){
        Integer userId = (Integer)request.getSession().getAttribute("userId");
        GoodsEntity goods = goodsService.selectById(ids);
        if(goods == null){
            return R.error();
        }
        if(goods.getAstrictNum() < num){
            return R.error(goods.getGoodsName()+" 这件物品最多可以领取 "+goods.getAstrictNum()+" "+goods.getDanwei());
        }
        if(goods.getGoodsNumber() < num){
            return R.error(goods.getGoodsName()+" 这件物品库存为 "+goods.getGoodsNumber()+" 暂时不够"+num+""+goods.getDanwei());
        }

        JiluEntity jilu = jiluService.selectOne(new EntityWrapper().eq("goods_id", goods.getId()).eq("yonghu_id", userId));
        if(jilu == null){
            JiluEntity jl = new JiluEntity();
            jl.setCreateTime(new Date());
            jl.setLingyongTime(new Date());
            jl.setGoodsId(ids);
            jl.setNum(num);
            jl.setYonghuId(userId);
            jiluService.insert(jl);
            goods.setGoodsNumber(goods.getGoodsNumber() - num);
        }else{
            if((jilu.getNum()+num) > goods.getAstrictNum()){
                return R.error("您已经领取了 "+jilu.getNum()+" "+goods.getDanwei()+""+goods.getGoodsName()+"了,再领取 "+num+" "+goods.getDanwei()+"会超出领取上限（"+goods.getAstrictNum()+"）");
            }
            jilu.setNum(jilu.getNum()+num);
